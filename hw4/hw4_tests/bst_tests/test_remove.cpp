...
TEST(BSTRemove, TwoChildrenRemove_3Node)
{
	...
	// succesor, 3, should be promoted
	ASSERT_NE((void*)nullptr, (void*)bst.root_);
	EXPECT_EQ(3, bst.root_->getKey());

	ASSERT_NE((void*)nullptr, (void*)bst.root_->getLeft());
	EXPECT_EQ(1, bst.root_->getLeft()->getKey());

	ASSERT_EQ((void*)nullptr, (void*)bst.root_->getRight());
...
TEST(BSTRemove, TwoChildrenRemove_4Node)
{
	...
	EXPECT_TRUE(verifyBST(bst, {2, 6, 7}));

	// predecessor, 4, should be promoted
	ASSERT_NE((void*)nullptr, (void*)bst.root_);
	EXPECT_EQ(6, bst.root_->getKey());

	ASSERT_NE((void*)nullptr, (void*)bst.root_->getRight());
	EXPECT_EQ(7, bst.root_->getRight()->getKey());
...
TEST(BSTStress, Remove)
{
	...
	testTree.remove(2);
#ifdef REMOVE_SWAP_PREDECESSOR
	EXPECT_EQ(1, testTree.root_->item_.first); //Predecessor Version
#else

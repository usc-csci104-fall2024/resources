...
//If remove swaps with predecessor leave this line uncommented
//If remove swaps with successor comment out this line
#define REMOVE_SWAP_PREDECESSOR
...
TEST(AVLRemove, SingleRemove)
{
	...
	avl.insert(std::make_pair(1, 1.0));
	avl.remove(1);
...
{
	AVLTree<int, double> avl;
	avl.insert(std::make_pair(3, 1.0));
	avl.insert(std::make_pair(4, 1.0));
	avl.insert(std::make_pair(2, 1.0));
	avl.insert(std::make_pair(1, 1.0));
	avl.remove(4);

	EXPECT_TRUE(verifyAVL(avl, {1, 2, 3}));
}
...
TEST(AVLRemove, LeftChildRemove)
{
	...
	EXPECT_TRUE(verifyAVL(avl, {2, 3, 4}));
}

TEST(AVLRemove, TwoChildrenRemove_3Node)
{
#ifdef REMOVE_SWAP_PREDECESSOR
	...
	EXPECT_TRUE(verifyAVL(avl, {1, 3}));

	// predecessor, 1, should be promoted
	ASSERT_NE((void*)nullptr, (void*)avl.root_);
	EXPECT_EQ(1, avl.root_->getKey());

	ASSERT_NE((void*)nullptr, (void*)avl.root_->getRight());
	EXPECT_EQ(3, avl.root_->getRight()->getKey());

	ASSERT_EQ((void*)nullptr, (void*)avl.root_->getLeft());
	...
	// successor, 3, should be promoted
	ASSERT_NE((void*)nullptr, (void*)avl.root_);
	EXPECT_EQ(3, avl.root_->getKey());

	ASSERT_NE((void*)nullptr, (void*)avl.root_->getLeft());
	EXPECT_EQ(1, avl.root_->getLeft()->getKey());

	ASSERT_EQ((void*)nullptr, (void*)avl.root_->getRight());
...
TEST(AVLRemove, TwoChildrenRemove_4Node)
{
	...
	AVLTree<int, double> avl;
	avl.insert(std::make_pair(4, 1.0));
	avl.insert(std::make_pair(5, 1.0));
	avl.insert(std::make_pair(2, 1.0));
	avl.insert(std::make_pair(6, 1.0));
	

	avl.remove(4);

	EXPECT_TRUE(verifyAVL(avl, {2, 6, 5}));

	// predecessor, 2, should be promoted, then rotation
	ASSERT_NE((void*)nullptr, (void*)avl.root_);
	EXPECT_EQ(5, avl.root_->getKey());

	ASSERT_NE((void*)nullptr, (void*)avl.root_->getRight());
	EXPECT_EQ(6, avl.root_->getRight()->getKey());

	ASSERT_NE((void*)nullptr, (void*)avl.root_->getLeft());
	EXPECT_EQ(2, avl.root_->getLeft()->getKey());
#else 
	AVLTree<int, double> avl;
	avl.insert(std::make_pair(5, 1.0));
	avl.insert(std::make_pair(4, 1.0));
	...
	EXPECT_TRUE(verifyAVL(avl, {2, 6, 4}));

	// succesor, 6, should be promoted, then rotation
	ASSERT_NE((void*)nullptr, (void*)avl.root_);
	EXPECT_EQ(4, avl.root_->getKey());
...
TEST(AVLRemove, TwoChildrenRemove_1ChildSwap)
{
#ifdef REMOVE_SWAP_PREDECESSOR
	AVLTree<int, double> avl;
	avl.insert(std::make_pair(5, 1.0));
	avl.insert(std::make_pair(2, 1.0));
	avl.insert(std::make_pair(6, 1.0));
	avl.insert(std::make_pair(7, 1.0));
	avl.insert(std::make_pair(1, 1.0));
	avl.insert(std::make_pair(4, 1.0));
	avl.insert(std::make_pair(3, 1.0));
	...
	EXPECT_TRUE(verifyAVL(avl, {1, 2, 3, 4,  6, 7}));
#else 
	AVLTree<int, double> avl;
	avl.insert(std::make_pair(3, 1.0));
	avl.insert(std::make_pair(2, 1.0));
	...
	EXPECT_TRUE(verifyAVL(avl, {1, 2, 4, 5, 6, 7}));

#endif
}

TEST(AVLRemove, TwoChildrenRemove_Unbalanced)
{
#ifdef REMOVE_SWAP_PREDECESSOR
	AVLTree<int, double> avl;
	avl.insert(std::make_pair(5, 1.0));
...
TEST(AVLStress, Remove)
{
	...
	testTree.remove(2);
#ifdef REMOVE_SWAP_PREDECESSOR
	EXPECT_EQ(10, testTree.root_->item_.first); //Predecessor Version
#else
	EXPECT_EQ(3, testTree.root_->item_.first); // Successor Version
#endif
